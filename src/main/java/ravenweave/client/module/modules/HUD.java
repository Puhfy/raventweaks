...
public class HUD extends Module {
    ...
    public void onRender2D(RenderGameOverlayEvent.Pre ev) {
        if (Utils.Player.isPlayerInGame()) {
            ...
            int margin = 2;
            int y = hudY + 3; // Shift down by 3 pixels
            int del = 0;
            ...
            if (hudY < margin)
                hudY = margin + 3; // Shift top margin by 3 pixels

            int screenWidth = mc.displayWidth;
            int screenHeight = mc.displayHeight;

            if ((hudX + textBoxWidth) > (screenWidth / 2))
                hudX = (screenWidth / 2) - textBoxWidth - margin;

            if ((hudY + textBoxHeight) > (screenHeight / 2))
                hudY = (screenHeight / 2) - textBoxHeight;

            int totalHeight = en.size() * (mc.fontRendererObj.FONT_HEIGHT + margin);

            for (int i = 0; i < en.size(); i++) {
                Module m = en.get(i);

                if (m.isEnabled() && m.showInHud()) {
                    int moduleHeight = mc.fontRendererObj.FONT_HEIGHT + margin;
                    int backgroundWidth = mc.fontRendererObj.getStringWidth(m.getName());
                    int backgroundColor = 0x80000000; // Transparent black

                    // Adjust the height of the rectangle based on its index
                    int rectHeight = moduleHeight;
                    if (i == 0) { // If it's the first rectangle
                        rectHeight -= 3; // Add 3px to its height
                    }

                    drawRect(hudX - 3, y + 3, hudX + backgroundWidth + 3, y + rectHeight + 3, backgroundColor);

                    if ((positionMode == Utils.HUD.PositionMode.DOWNRIGHT) || (positionMode == Utils.HUD.PositionMode.UPRIGHT)) {
                        switch (colorMode.getMode()) {
                            case PUHFY:
                                mc.fontRendererObj.drawString(m.getName() + " " + m.getSettings(),
                                        (float) hudX + (textBoxWidth - mc.fontRendererObj.getStringWidth(m.getName())),
                                        (float) y + 3 , Utils.Client.drawColor(255, 255, 255), dropShadow.isToggled());
                        ...
                        y += mc.fontRendererObj.FONT_HEIGHT + margin;
                        del -= 10;
                    } else if (colorMode.getMode() == ColourModes.PUHFY) {
                        mc.fontRendererObj.drawString(m.getName(), (float) hudX, (float) y + 3,
                                Utils.Client.drawColor(255, 255, 255), dropShadow.isToggled());
                        y += mc.fontRendererObj.FONT_HEIGHT + margin;
                        del -= 10;
    ...
    private void drawLogo(int e) {
        ...
        if (logoLoaded()) {
            ...
            mc.getMinecraft().getTextureManager().bindTexture(ravenLogo);
    ...
    static class EditHudPositionScreen extends GuiScreen {
        ...
        private void drawArrayList(FontRenderer fr, String t) {
            ...
            if ((positionMode == Utils.HUD.PositionMode.DOWNRIGHT) || (positionMode == Utils.HUD.PositionMode.UPRIGHT))
                for (String s : var5) {
                    int textWidth = fr.getStringWidth(s);
                    int textHeight = fr.FONT_HEIGHT;
                    int startX = x + (gap - textWidth) / 2; // Center the text horizontally
                    int startY = (int) (y + (marginY - textHeight) / 2); // Center the text vertically
                    fr.drawString(s, (float) startX, (float) startY, Color.white.getRGB(), dropShadow.isToggled());
                    y += marginY;
                }
            else
                for (String s : var5) {
    ...
    public enum ColourModes {
        PUHFY, RAVEN, RAVEN2, RAVENB4, ASTOLFO, ASTOLFO2, ASTOLFO3, KV
    }
